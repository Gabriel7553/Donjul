...
const addMonth = (dateStr, months = 1) => { const d = new Date(dateStr + 'T00:00:00'); d.setMonth(d.getMonth() + months); return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`; };

function diffDays(toDate, fromDate = todayStr()) {
...
const ICON_MAP = { languages: Languages, music: Music, shield: Shield, book: BookOpen, target: Target, dumbbell: Dumbbell, activity: Activity, run: Activity };
...
const DEFAULT_SETTINGS = {
  ...
  // Schedule config
  scheduleStartOffsetMin: 30,   // default minutes after wake to start studying
  blockBreakMin: 15,            // break between schedule blocks
  // Macros
  ...
  challenge: {
    ...
    startDate: null,
    days: 60,
    deloadWeek: 5,        // 1-indexed week (Mon-Sun) where intensity drops
  },
};
...
// PROGRESS MATH
// ════════════════════════════════════════════════════════════════════════════════
function expectedTotal(subjectKey, settings) {
...
function targetTotalByDeadline(subjectKey, settings) {
  const s = settings.subjects[subjectKey];
  if (!s) return 0;
...
export default function App() {
  ...
  const [body, setBody] = useState({ entries: [] });
  const [workout, setWorkout] = useState({ split: DEFAULT_WORKOUT_SPLIT, logs: {}, mode: 'sequence', sequencePosition: 1 });
  const [meals, setMeals] = useState({ presets: [], log: {} });
  const [plans, setPlans] = useState({});
  const [streaks, setStreaks] = useState({});
  const [weeklyAck, setWeeklyAck] = useState({ lastAck: null });
  const [modal, setModal] = useState(null);

  useEffect(() => {
    (async () => {
      const s = await safeGet(K.settings, DEFAULT_SETTINGS);
      // Migrate / hydrate any missing fields
      ...
      // Hydrate mode/sequencePosition for older saves
      ...
      if (!d || d.date !== todayStr()) {
        ...
        d = {
          ...
          scheduleStarted: false,    // user has confirmed when to start studying
          scheduleStartTime: null,   // actual time to start blocks
          status: 'home',
          busyUntil: null,
          completed: Object.fromEntries(Object.keys(merged.subjects).map(k => [k, 0])),
          bonus: Object.fromEntries(Object.keys(merged.subjects).map(k => [k, 0])),
        };
        await safeSet(K.daily, d);
      }
      // Hydrate any missing fields on existing daily
      if (d.scheduleStarted == null) d.scheduleStarted = false;
      if (!d.bonus) d.bonus = Object.fromEntries(Object.keys(merged.subjects).map(k => [k, 0]));
      setSettings(merged);
      setDaily(d);
      setTotals(t);
      setBody(b);
  ...
  // Save helpers
  const saveSettings = async (next) => { setSettings(next); await safeSet(K.settings, next); };
  const saveDaily = async (next) => { setDaily(next); await safeSet(K.daily, next); };
  const saveTotals = async (next) => { setTotals(next); await safeSet(K.totals, next); };
  const saveBody = async (next) => { setBody(next); await safeSet(K.body, next); };
  const saveWorkout = async (next) => { setWorkout(next); await safeSet(K.workout, next); };
  const saveMeals = async (next) => { setMeals(next); await safeSet(K.meals, next); };
  const savePlans = async (next) => { setPlans(next); await safeSet(K.plans, next); };
  const saveStreaks = async (next) => { setStreaks(next); await safeSet(K.streaks, next); };

  const logTime = async (subject, minutes) => {
  ...
  if (!settings.setupComplete) {
    return <Setup onComplete={(s) => saveSettings({ ...s, setupComplete: true })} onImport={async (d) => {
  ...
  // Weekly review trigger — show only on Sunday if not yet acknowledged today
  ...
  return (
    <div className="app">
      ...
      <div className="content">
        ...
        {tab === 'today' && (
          <TodayTab
            ...
            onWake={(time) => saveDaily({ ...daily, wakeLogged: true, actualWake: time })}
            onScheduleStart={(time) => saveDaily({ ...daily, scheduleStarted: true, scheduleStartTime: time })}
            onStatus={(status, busyUntil) => saveDaily({ ...daily, status, busyUntil })}
            onLogTime={(s) => setModal({ type: 'logTime', subject: s })}
        ...
        {tab === 'workout' && (
          <WorkoutTab
            ...
            onLogWorkout={(idx) => setModal({ type: 'logWorkout', dayIdx: idx })}
        ...
        {tab === 'plan' && (
          <PlanTab
            ...
            onPlanDay={(date) => setModal({ type: 'planDay', date })}
          />
        )}
        {tab === 'history' && (
          <HistoryTab
            settings={settings} totals={totals} workout={workout} meals={meals} body={body}
            onSelectDay={(date) => setModal({ type: 'dayDetail', date })}
          />
        )}
      </div>
      ...
      {/* Modals */}
      {modal?.type === 'settings' && <SettingsModal settings={settings} onSave={saveSettings} onClose={() => setModal(null)} onEditSubject={(k) => setModal({ type: 'editSubject', key: k })} onAddSubject={() => setModal({ type: 'editSubject', key: null })} onChallenge={() => setModal({ type: 'challenge' })} onExportImport={() => setModal({ type: 'exportImport' })} />}
      ...
      {modal?.type === 'logTime' && <LogTimeModal subject={modal.subject} settings={settings} daily={daily} onLog={(m) => { logTime(modal.subject, m); setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === 'busy' && <BusyModal onConfirm={(m) => { saveDaily({ ...daily, status: 'busy', busyUntil: addMinutes(nowHHMM(), m) }); setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === 'addMeasurement' && <AddMeasurementModal onSave={async (entry) => { const next = { ...body, entries: [...body.entries, entry] }; const nextSettings = { ...settings, nextMeasurement: addMonth(todayStr(), 1) }; await saveBody(next); await saveSettings(nextSettings); setModal(null); }} onClose={() => setModal(null)} previous={body.entries[body.entries.length - 1]} />}
      ...
      {modal?.type === 'exportImport' && <ExportImportModal data={{ settings, totals, body, workout, meals, plans, streaks, weeklyAck }} onImport={async (d) => {
        if (d.settings) { setSettings(d.settings); await safeSet(K.settings, d.settings); }
        if (d.totals) { setTotals(d.totals); await safeSet(K.totals, d.totals); }
        if (d.body) { setBody(d.body); await safeSet(K.body, d.body); }
        if (d.workout) { setWorkout(d.workout); await safeSet(K.workout, d.workout); }
...
// ════════════════════════════════════════════════════════════════════════════════
// SAMPLE PRESETS (starter meals; user replaces with their actual go-tos)
// ════════════════════════════════════════════════════════════════════════════════
...
// HEADER + BOTTOM NAV
// ════════════════════════════════════════════════════════════════════════════════
function Header({ date, onSettings }) {
  return (
...
// TODAY TAB
// ════════════════════════════════════════════════════════════════════════════════
function TodayTab({ settings, daily, totals, streaks, meals, workout, onWake, onStatus, onLogTime, onBusy, onLogMeal, onScheduleStart }) {
  const subjectKeys = settings.subjectOrder.filter(k => settings.subjects[k] && !settings.subjects[k].archived);
...
function WakeCheckIn({ plannedWake, onLog }) {
  ...
  return (
    <div className="card" style={{ borderLeft: '3px solid #B8460E' }}>
      ...
      <p style={{ fontSize: 16, marginBottom: 14, lineHeight: 1.4 }}>
...
function ScheduleStartCard({ daily, defaultOffset, onStart }) {
  ...
  return (
    <div className="card" style={{ borderLeft: '3px solid #B8460E' }}>
      ...
      <p style={{ fontSize: 15, marginBottom: 6, lineHeight: 1.4 }}>
        You woke at <span className="mono">{fmtTime(daily.actualWake)}</span>. Is now a good time to start studying, or do you need a bit?
      </p>
      <p className="muted tiny" style={{ marginBottom: 14 }}>
        Pick when your first block should begin. You can change it later.
      </p>
...
function StatusBar({ daily, onBusy, onHome }) {
...
function Schedule({ settings, daily, onLog, subjectKeys }) {
...
function buildSchedule(settings, daily, subjectKeys) {
  if (!daily?.scheduleStartTime) return [];
  const breakMin = settings.blockBreakMin ?? 15;
  let cursor = daily.scheduleStartTime;
  const blocks = [];
  const dow = dayOfWeek();
  // Ordering: cysa (deadline urgency) → running (do early, frees evening) → spanish (largest) → guitar (evening)
  ...
  for (const k of order) {
    ...
    if (k === 'spanish' && remaining >= 60) {
      // 3-block split: Duolingo, Babbel, Input (spread through day)
      ...
      // Second block ~90 min later (after lunch slot)
      ...
      // Third block (input) later afternoon
      ...
      // Running fits well early; if no scheduled gym, do it now. Otherwise late afternoon.
...
function splitSpanish(total) {
  // Duolingo ~42%, Babbel ~37%, Input ~21%
...
function Progress({ settings, totals, daily, streaks, subjectKeys, onLogExtra }) {
  return (
    <div className="card">
      ...
      {subjectKeys.map((k) => {
        ...
        const daysLeft = diffDays(subj.deadline);
      ...
      <div className="muted tiny" style={{ lineHeight: 1.4, marginTop: 4 }}>
        Notch shows where you should be today. Tap "+ log" anytime to add time, even after a target's hit.
      </div>
    </div>
  );
}

function MacrosCard({ targets, totals, onLog }) {
  const items = [
  ...
  return (
    <div className="card">
      ...
      {items.map(it => {
        ...
        return (
          <div key={it.key} style={{ marginBottom: 10 }}>
            <div className="between" style={{ marginBottom: 4 }}>
              <span className="small" style={{ fontWeight: it.primary ? 600 : 400 }}>
                {it.label}{it.primary && ' ★'}
              </span>
      ...
      <div className="muted tiny" style={{ lineHeight: 1.4, marginTop: 4 }}>
        ★ Protein is your priority. Hit that first.
      </div>
    </div>
  );
}

function ChallengeCard({ settings, workout }) {
  ...
  // Count workouts logged within challenge window
  const sessions = Object.keys(workout.logs || {}).filter(d => {
...
function WeeklySummary({ settings, totals, daily, meals, workout }) {
  const subjectKeys = settings.subjectOrder.filter(k => settings.subjects[k] && !settings.subjects[k].archived);
  const week = useMemo(() => {
    const days = [];
    for (let i = 6; i >= 0; i--) {
  ...
  const workoutsThisWeek = week.filter(d => workout.logs?.[d]).length;
  // Average protein over last 7 days that have entries
  const proteinDays = week.map(d => meals.log?.[d]?.protein).filter(x => x != null && x > 0);
  const avgProtein = proteinDays.length > 0 ? proteinDays.reduce((a, b) => a + b, 0) / proteinDays.length : 0;
  ...
  return (
    <div className="card">
      ...
      <div className="muted tiny" style={{ marginTop: 8 }}>
        Filled dots = workout day. Tap History (in Settings) for full daily breakdowns.
      </div>
    </div>
  );
}
...
// BODY TAB
// ════════════════════════════════════════════════════════════════════════════════
function BodyTab({ settings, body, workout, onAddEntry, onEditGoals }) {
  ...
  // Recomp score: weighted progress across measurements (needs 2+ entries to mean anything)
  const recompScore = useMemo(() => {
    if (entries.length < 2 || !latest) return null;
    const baseline = entries[0];
    let totalWeight = 0;
    let progressSum = 0;
    Object.entries(settings.bodyGoals).forEach(([k, g]) => {
      ...
      const signed = delta * direction; // positive = good
      // Normalize: percent change relative to start
      ...
      {entries.length === 0 ? (
        <div className="card">
          ...
          <p className="small muted" style={{ lineHeight: 1.5, marginBottom: 12 }}>
            Tap "Log" above to enter your first set tomorrow. Going forward, monthly on the same date.
          </p>
...
// WORKOUT TAB
// ════════════════════════════════════════════════════════════════════════════════
function WorkoutTab({ workout, onLogWorkout, onEditSplit, onSaveWorkout, settings }) {
  const today = dayOfWeek();
  // Determine today's workout based on mode
  const mode = workout.mode || 'sequence';
  let todayWorkout;
  let todayWorkoutIdx;
  if (mode === 'calendar') {
    todayWorkout = workout.split.find(w => w.day === today) || workout.split[0];
    todayWorkoutIdx = workout.split.indexOf(todayWorkout);
  } else {
    // Sequence mode: show the next non-rest day in sequence
    const nonRestDays = workout.split.filter(d => !d.rest);
  ...
  // Last logged workout for "previous session" comparison
  const recentLogs = Object.entries(workout.logs).sort((a, b) => b[0].localeCompare(a[0])).slice(0, 5);

  // Last weights per exercise (progressive overload helper)
  const lastWeights = useMemo(() => {
    const map = {};
    const sortedLogs = Object.entries(workout.logs).sort((a, b) => b[0].localeCompare(a[0]));
    for (const [date, log] of sortedLogs) {
      if (!log.exercises) continue;
      for (const ex of log.exercises) {
        if (map[ex.name]) continue;
        const lastSet = ex.sets?.filter(s => s.weight).pop();
      ...
      {todayWorkout.rest ? (
        <div className="card">
          ...
          <p className="muted small" style={{ lineHeight: 1.5 }}>
            Recovery. Light walking, stretching, or mobility work if you want it. Sleep + protein matter more today than anything in the gym.
          </p>
        </div>
      ) : (
          ...
          <div className="between" style={{ marginBottom: 14 }}>
            <div>
              <div className="h2" style={{ marginBottom: 2 }}>
                {mode === 'sequence' ? `Next up` : `Day ${todayWorkout.day} · ${new Date(todayStr() + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'short' })}`}
              </div>
              ...
              <div key={i} style={{ padding: '10px 0', borderBottom: i < todayWorkout.exercises.length - 1 ? '1px solid #E4DCC8' : 'none' }}>
                ...
                {logged?.sets?.length > 0 && (
                  <div className="mono tiny muted" style={{ marginTop: 4 }}>
                    {logged.sets.map((s, j) => `${s.weight || '—'}×${s.reps || '—'}`).join('  ')}
                  </div>
                )}
              </div>
            );
          ...
          <div key={i} className="between" style={{ padding: '8px 0', borderBottom: i < workout.split.length - 1 ? '1px solid #E4DCC8' : 'none', opacity: (mode === 'calendar' && day.day === today) || (mode === 'sequence' && day === todayWorkout) ? 1 : 0.7 }}>
            <div className="row" style={{ gap: 10 }}>
              <span className="mono tiny muted">
                {mode === 'calendar' ? ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][day.day] : day.rest ? 'REST' : `D${workout.split.filter(d => !d.rest).indexOf(day) + 1}`}
              </span>
...
// PLAN TAB
// ════════════════════════════════════════════════════════════════════════════════
function PlanTab({ plans, settings, onPlanDay }) {
  const today = todayStr();
  const upcoming = [];
  for (let i = 0; i < 7; i++) {
...
function PlanRow({ date, plans, onClick, highlight }) {
  const p = plans[date];
...
// MODALS
// ════════════════════════════════════════════════════════════════════════════════
function HistoryTab({ settings, totals, workout, meals, body, onSelectDay }) {
  ...
  const proteinValues = monthDays.map(d => meals.log?.[d]?.protein).filter(x => x != null && x > 0);
  ...
  return (
    <>
      ...
      <div className="card">
        <div className="between" style={{ marginBottom: 14 }}>
          <button onClick={goPrev} className="tap" style={{ padding: 6 }}>
            <ChevronLeft size={16} />
          </button>
          <div className="h3">{monthName}</div>
          <button onClick={goNext} className="tap" style={{ padding: 6 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, marginBottom: 14 }}>
          ...
          {monthDays.map(d => {
            ...
            const hasMeasurement = body.entries?.some(e => e.date === d);
            const isToday = d === todayStr();
      ...
      <p className="muted tiny" style={{ textAlign: 'center', marginTop: 8 }}>
        Tap any past day for the full breakdown.
      </p>
    </>
  );
}

function DayDetailModal({ date, settings, totals, workout, meals, body, onClose }) {
  const dayMeals = meals.log?.[date];
  const dayWorkout = workout.logs?.[date];
  const dayMeasurement = body.entries?.find(e => e.date === date);
      ...
      {dayMeals ? (
        <div className="card" style={{ padding: 12, marginBottom: 14 }}>
          <div className="row" style={{ gap: 16, flexWrap: 'wrap' }}>
            <div>
              <div className="mono tiny muted">PROTEIN</div>
              <div className="h3" style={{ color: '#B8460E' }}>{Math.round(dayMeals.protein || 0)}g</div>
            </div>
            <div>
              <div className="mono tiny muted">CALORIES</div>
              <div className="h3">{Math.round(dayMeals.calories || 0)}</div>
            </div>
            <div>
              <div className="mono tiny muted">CARBS</div>
              <div className="h3">{Math.round(dayMeals.carbs || 0)}g</div>
            </div>
            <div>
              <div className="mono tiny muted">FAT</div>
              <div className="h3">{Math.round(dayMeals.fat || 0)}g</div>
            </div>
          </div>
        </div>
      ) : (
            ...
            <div key={i} style={{ padding: '6px 0', borderBottom: i < dayWorkout.exercises.length - 1 ? '1px solid #E4DCC8' : 'none' }}>
              ...
              {ex.sets?.length > 0 && (
                <div className="mono tiny muted">
                  {ex.sets.filter(s => s.weight || s.reps).map(s => `${s.weight || '—'}×${s.reps || '—'}${s.rpe ? ` @${s.rpe}` : ''}`).join('  ·  ')}
                </div>
              )}
            </div>
          ))}
...
function ModalShell({ title, onClose, children, icon = null, color = '#1A1A2E' }) {
  return (
...
function LogTimeModal({ subject, settings, daily, onLog, onClose }) {
...
function BusyModal({ onConfirm, onClose }) {
  const [mins, setMins] = useState(60);
  return (
    <ModalShell title="Stepping out" onClose={onClose} icon={<Footprints size={18} color="#C8932E" />}>
      <p className="muted small" style={{ marginBottom: 14 }}>
        I'll shift remaining blocks. Tap "I'm back" when you return — even if longer than this.
      </p>
...
function AddMeasurementModal({ onSave, onClose, previous }) {
  ...
  return (
    <ModalShell title="Log measurements" onClose={onClose} icon={<Activity size={18} color="#B8460E" />}>
      <p className="muted small" style={{ marginBottom: 14 }}>
        Leave blank what you don't measure. Use the same scale and tape each time for honest comparisons.
      </p>
      {MEASUREMENT_FIELDS.map(f => (
...
function BodyGoalsModal({ settings, onSave, onClose }) {
  const [goals, setGoals] = useState(settings.bodyGoals);
  const set = (k, patch) => setGoals({ ...goals, [k]: { ...goals[k], ...patch } });

  return (
    <ModalShell title="Body targets" onClose={onClose} icon={<Target size={18} color="#B8460E" />}>
      <p className="muted small" style={{ marginBottom: 14 }}>
        Set target numbers if you want progress bars to show toward a specific goal. Direction is for trend coloring.
      </p>
      {MEASUREMENT_FIELDS.map(f => {
        const g = goals[f.key];
        if (!g) return null;
...
function LogWorkoutModal({ dayIdx, workout, onSave, onClose }) {
  const day = workout.split[dayIdx];
  const today = todayStr();
  const initial = workout.logs[today] || { name: day.name, exercises: day.exercises.map(ex => ({ name: ex.name, sets: Array(ex.sets).fill(0).map(() => ({ weight: '', reps: '', rpe: '' })) })) };
  const [data, setData] = useState(initial);

  const setSet = (exIdx, setIdx, field, value) => {
    const next = { ...data, exercises: data.exercises.map((e, i) => i !== exIdx ? e : { ...e, sets: e.sets.map((s, j) => j !== setIdx ? s : { ...s, [field]: value }) }) };
    setData(next);
  };
  ...
  return (
    <ModalShell title={day.name} onClose={onClose} icon={<Dumbbell size={18} color="#3B5C6B" />}>
      ...
      <button className="btn" style={{ width: '100%' }} onClick={async () => {
        ...
        // Auto-advance sequence position on first log of the day in sequence mode
...
function EditSplitModal({ workout, onSave, onClose }) {
  const [split, setSplit] = useState(workout.split);
  const [openDay, setOpenDay] = useState(null);
  const updateDay = (idx, patch) => setSplit(split.map((d, i) => i === idx ? { ...d, ...patch } : d));
  const updateExercise = (dayIdx, exIdx, patch) => updateDay(dayIdx, { exercises: split[dayIdx].exercises.map((e, i) => i === exIdx ? { ...e, ...patch } : e) });
  const addExercise = (dayIdx) => updateDay(dayIdx, { exercises: [...split[dayIdx].exercises, { name: 'New exercise', sets: 3, reps: '8-10', weight: '' }] });
  const removeExercise = (dayIdx, exIdx) => updateDay(dayIdx, { exercises: split[dayIdx].exercises.filter((_, i) => i !== exIdx) });
...
function LogMealModal({ meals, settings, onSave, onClose }) {
  const [mode, setMode] = useState('preset'); // 'preset' | 'manual' | 'add'
  ...
  const addPreset = async () => {
    const p = {
      id: 'p' + Date.now(),
      name: newPreset.name,
      protein: parseFloat(newPreset.protein) || 0,
      carbs: parseFloat(newPreset.carbs) || 0,
      fat: parseFloat(newPreset.fat) || 0,
      calories: parseFloat(newPreset.calories) || 0,
  ...
  const removePreset = async (id) => {
    await onSave({ ...meals, presets: meals.presets.filter(p => p.id !== id) });
  };
      ...
      {mode === 'preset' && (
        <>
          <div className="muted tiny" style={{ marginBottom: 10 }}>
            Tap a meal to log it. Add Skinnytaste recipes via "+ New preset".
          </div>
      ...
      {mode === 'add' && (
        <>
          ...
          <div className="row" style={{ gap: 8 }}>
            <div style={{ flex: 1 }}>
              <label>Protein</label>
              <input type="number" value={newPreset.protein} onChange={(e) => setNewPreset({ ...newPreset, protein: e.target.value })} />
            </div>
            <div style={{ flex: 1 }}>
              <label>Carbs</label>
              <input type="number" value={newPreset.carbs} onChange={(e) => setNewPreset({ ...newPreset, carbs: e.target.value })} />
            </div>
          </div>
          <div className="row" style={{ gap: 8, marginTop: 10 }}>
            <div style={{ flex: 1 }}>
              <label>Fat</label>
              <input type="number" value={newPreset.fat} onChange={(e) => setNewPreset({ ...newPreset, fat: e.target.value })} />
            </div>
            <div style={{ flex: 1 }}>
              <label>Calories</label>
              <input type="number" value={newPreset.calories} onChange={(e) => setNewPreset({ ...newPreset, calories: e.target.value })} />
            </div>
          </div>
...
function PlanDayModal({ date, plans, onSave, onClose }) {
    ...
    <ModalShell title={fmtDate(date)} onClose={onClose} icon={<CalIcon size={18} color="#B8460E" />}>
      ...
      {commitments.map((c, i) => (
        <div key={i} className="between" style={{ padding: '8px 10px', background: '#FBF7EE', border: '1px solid #E4DCC8', borderRadius: 8, marginBottom: 6 }}>
          ...
          <button onClick={() => setCommitments(commitments.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B8460E' }}>
            <X size={14} />
          </button>
        </div>
      ))}
      ...
      {tasks.map((t, i) => (
        <div key={i} className="between" style={{ padding: '8px 10px', background: '#FBF7EE', border: '1px solid #E4DCC8', borderRadius: 8, marginBottom: 6 }}>
          ...
          <button onClick={() => setTasks(tasks.filter((_, j) => j !== i))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#B8460E' }}>
            <X size={14} />
          </button>
        </div>
      ))}
...
function SettingsModal({ settings, onSave, onClose, onEditSubject, onAddSubject, onChallenge, onExportImport }) {
  ...
  return (
    <ModalShell title="Settings" onClose={onClose}>
      ...
      {draft.subjectOrder.map(k => {
        const s = draft.subjects[k];
        if (!s) return null;
        const Icon = ICON_MAP[s.icon] || Languages;
        return (
          <div key={k} className="between" style={{ padding: '10px 12px', background: s.archived ? '#F0EBDE' : '#FBF7EE', border: '1px solid #E4DCC8', borderRadius: 8, marginBottom: 6, cursor: 'pointer' }} onClick={() => onEditSubject(k)}>
            <div className="row" style={{ gap: 10 }}>
              <div className="icon-wrap" style={{ background: s.accent, width: 28, height: 28 }}><Icon size={14} /></div>
              <div>
                <div className="small" style={{ fontWeight: 600, textDecoration: s.archived ? 'line-through' : 'none' }}>{s.name}</div>
                <div className="muted tiny mono">{s.target}m × {s.weeklyDays}d/wk · ends {fmtShortDate(s.deadline)}</div>
              </div>
            </div>
            <ChevronRight size={16} color="#6B6457" />
          </div>
      ...
      <button className="tap" onClick={onAddSubject} style={{ width: '100%', marginTop: 6 }}>
        <Plus size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} /> Add subject
      </button>

      <div className="h2" style={{ marginBottom: 8, marginTop: 20 }}>Macro targets</div>
      <div className="row" style={{ gap: 8, marginBottom: 10 }}>
        <div style={{ flex: 1 }}>
          <label>Protein (g)</label>
          <input type="number" value={draft.macroTargets.protein} onChange={(e) => update({ macroTargets: { ...draft.macroTargets, protein: parseInt(e.target.value) || 0 } })} />
        </div>
        <div style={{ flex: 1 }}>
          <label>Calories</label>
          <input type="number" value={draft.macroTargets.calories} onChange={(e) => update({ macroTargets: { ...draft.macroTargets, calories: parseInt(e.target.value) || 0 } })} />
        </div>
      </div>
      <div className="row" style={{ gap: 8, marginBottom: 16 }}>
        <div style={{ flex: 1 }}>
          <label>Carbs (g)</label>
          <input type="number" value={draft.macroTargets.carbs} onChange={(e) => update({ macroTargets: { ...draft.macroTargets, carbs: parseInt(e.target.value) || 0 } })} />
        </div>
        <div style={{ flex: 1 }}>
          <label>Fat (g)</label>
          <input type="number" value={draft.macroTargets.fat} onChange={(e) => update({ macroTargets: { ...draft.macroTargets, fat: parseInt(e.target.value) || 0 } })} />
        </div>
      </div>

      <div className="h2" style={{ marginBottom: 8 }}>Body</div>
      <div style={{ marginBottom: 16 }}>
        <label>Next measurement</label>
        <input type="date" value={draft.nextMeasurement || ''} onChange={(e) => update({ nextMeasurement: e.target.value })} />
      </div>

      <button className="btn" style={{ width: '100%', marginBottom: 14 }} onClick={() => { onSave(draft); onClose(); }}>
        <Save size={14} style={{ verticalAlign: 'middle', marginRight: 6 }} /> Save settings
      </button>

      <hr className="divider" />

      <div className="h2" style={{ marginBottom: 8 }}>More</div>
      <button className="tap" onClick={onChallenge} style={{ width: '100%', marginBottom: 8, textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ flex: 1 }}>
          60-day challenge
          {settings.challenge?.active && <span className="mono tiny muted" style={{ marginLeft: 6 }}>· active</span>}
        </span>
...
function EditSubjectModal({ subjectKey, settings, onSave, onClose }) {
    ...
    <ModalShell title={isNew ? 'New subject' : 'Edit subject'} onClose={onClose}>
      ...
      <div className="row" style={{ gap: 8, marginBottom: 12 }}>
        <div style={{ flex: 1 }}>
          ...
          <input type="number" value={draft.target} onChange={(e) => setDraft({ ...draft, target: parseInt(e.target.value) || 0 })} />
        </div>
        <div style={{ flex: 1 }}>
          <label>Days/week</label>
          <input type="number" min="1" max="7" value={draft.weeklyDays} onChange={(e) => setDraft({ ...draft, weeklyDays: parseInt(e.target.value) || 1 })} />
        </div>
      </div>
...
function WeeklyReviewModal({ settings, totals, body, workout, streaks, onAck, onSkip }) {
  const subjects = settings.subjectOrder.filter(k => settings.subjects[k] && !settings.subjects[k].archived);
  return (
...
function ChallengeModal({ settings, onSave, onClose }) {
  ...
  return (
    <ModalShell title="60-Day Challenge" onClose={onClose} icon={<Zap size={18} color="#8E4585" />}>
      <p className="muted small" style={{ marginBottom: 14, lineHeight: 1.5 }}>
        A focused training block with a built-in deload week for recovery. Tracks separately from your overall progress.
      </p>
      ...
      <div className="row" style={{ gap: 8, marginBottom: 12 }}>
        <div style={{ flex: 1 }}>
          ...
          <input type="number" value={draft.days} onChange={(e) => setDraft({ ...draft, days: parseInt(e.target.value) || 60 })} />
        </div>
        <div style={{ flex: 1 }}>
          <label>Deload week #</label>
          <input type="number" min="1" max="10" value={draft.deloadWeek} onChange={(e) => setDraft({ ...draft, deloadWeek: parseInt(e.target.value) || 5 })} />
        </div>
      </div>
      <p className="muted tiny" style={{ marginBottom: 14, lineHeight: 1.4 }}>
        Default: week 5 of 9 (roughly the middle). During deload, drop weights ~30% and focus on form. Your body needs the break to grow.
      </p>
...
function ExportImportModal({ data, onImport, onClose }) {
      ...
      {mode === 'export' ? (
        <>
          <p className="muted small" style={{ marginBottom: 12, lineHeight: 1.5 }}>
            Copy this and save it anywhere (Notes, Files, email to yourself). If the app ever clears, paste it back into Import to restore everything.
          </p>
          <textarea readOnly value={exportJson} style={{ fontFamily: 'JetBrains Mono', fontSize: 10, padding: 10, height: 200, marginBottom: 12, resize: 'vertical', width: '100%' }} onClick={(e) => e.target.select()} />
...
// ════════════════════════════════════════════════════════════════════════════════
function Setup({ onComplete, onImport }) {
  const [step, setStep] = useState(0);
  const [draft, setDraft] = useState({ ...DEFAULT_SETTINGS, startDate: todayStr() });
  const [showImport, setShowImport] = useState(false);
  const [importText, setImportText] = useState('');
  const [importError, setImportError] = useState('');
  const update = (patch) => setDraft({ ...draft, ...patch });
  const updateSubject = (k, patch) => update({ subjects: { ...draft.subjects, [k]: { ...draft.subjects[k], ...patch } } });
    ...
    },
    {
      ...
      content: (
        <>
          ...
          <div className="muted tiny" style={{ marginTop: 10, lineHeight: 1.4 }}>
            Defaults from your fitness chat: 170p / 2300cal for recomp. Edit anytime.
          </div>
        </>
      ), next: 'Next',
    },
  ...
  return (
    <div className="app">
      ...
      <div className="content" style={{ paddingTop: 40 }}>
        ...
        {/* Restore from backup — only on welcome screen */}
        {step === 0 && (
          <>
